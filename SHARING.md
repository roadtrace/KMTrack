...
- Use **Import** beside **Select** in the inspection log. Choose an original SPOT IT Excel (.xlsx) or Photos ZIP (.zip).
...
Original older SPOT IT workbooks and version-1 Photos ZIP files remain importable. Older workbooks lack stable IDs, names, full photo metadata, and sub-meter KM precision. Their KM column is converted from meters back into internal kilometers. Missing data cannot be reconstructed.
...
Use original SPOT IT exports, not arbitrary spreadsheets. If inspection cells no longer match a new workbook's sharing details, import rejects it rather than silently using stale data. Compressed files require browser support for raw-deflate decompression; original SPOT IT exports use uncompressed ZIP entries.
