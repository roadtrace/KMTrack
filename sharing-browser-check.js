async (page) => {
  ...
  await page.evaluate(async()=>{
    ...
    for(const key of await caches.keys())if(key.startsWith('spotit-shell-')||key.startsWith('kmtrack-shell-'))await caches.delete(key);
  });
  await page.reload();
  const result=await page.evaluate(async()=>{
    ...
    const parsed=await SPOTITSharing.readImport(file);
    if(parsed.rows[0].km!==row.km||parsed.rows[0].inspector!=='Juan')throw Error('Excel round-trip mismatch');
    const legacyFiles=[...await SPOTITSharing.readZip(workbook)].filter(([path])=>path!=='xl/worksheets/sheet2.xml');
    const legacy=await createStoredZip(await Promise.all(legacyFiles.map(async([name,part])=>({name,blob:await part.blob()}))));
    const old=await SPOTITSharing.readImport(new File([legacy],'Old.xlsx'));
